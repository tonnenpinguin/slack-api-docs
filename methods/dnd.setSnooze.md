...
HTTP
`GEThttps://slack.com/api/dnd.setSnooze`
...
[User tokens](/docs/token-types#user)[`dnd:write`](/scopes/dnd:write)
...
Arguments `num_minutes` and `is_indefnite` are mutually exclusive and one of them is always required.
...
No value provided for `num_minutes`
 |
| `cannot_set_both_num_minutes_and_is_indefinite` | 
Cannot set both `num_minutes` and `is_indefinite` at the same time
 |
| `not_authed` | 
