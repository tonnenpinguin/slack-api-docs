...
HTTP
`GEThttps://slack.com/api/auth.revoke`
...
If your app uses a [workspace token](/docs/token-types#workspace), consider using a [bot token](/docs/token-types#bot) instead! If the token is revoked without further side effects: the app is not uninstalled. To uninstall a workspace app, use the [`app.uninstall`](/methods/app.uninstall) method.

If you have an org wide app, in order to completely remove your app from a workspace or org, an org admin must do so in the admin console.
