...
HTTP
`POSThttps://slack.com/api/admin.auth.policy.removeEntities`
...
This [Admin API method](/admins) removes entities (currently, users) from an authentication policy—for example, signing in with email and password. After being removed, the entities will authenticate using the default policy on your organization, which might be a policy like Single-Sign On.
