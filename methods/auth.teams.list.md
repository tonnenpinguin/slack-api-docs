...
HTTP
`GEThttps://slack.com/api/auth.teams.list`
...
This API method lists the workspaces a [token](/authentication/token-types) has access to.

Most tokens are tied to a single workspace.

However, if your [app works with Enterprise Grid](/enterprise/apps), it may be installed across the entire organization. In that case, this method returns _all_ individual workspaces that the token has been granted access to within that organization.
